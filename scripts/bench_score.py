...
a check that could not run. That rule was stated here before it was implemented:
`unchanged`/`added`/`removed` are `None` on a one-sided table and the arithmetic
raised on the first real pair, which is a crash rather than the silent miss it was
guarding against, but it is the same defect.
"""
