...
async def test_validate_passes_on_a_complete_spec(essentials_client, spec_dir):
...
async def test_validate_reports_the_mode_it_answered_for(essentials_client, spec_dir):
    """A validation verdict is only meaningful paired with its mode."""
    lenient = await essentials_client.call_tool(
        "validate_module", {"spec_dir": str(spec_dir), "strict": False}
    )
    assert lenient.data.strict is False


async def test_validate_refuses_an_unreplaced_placeholder(essentials_client, tmp_path):
    spec = tmp_path / "spec"
    spec.mkdir()
...
async def test_missing_spec_dir_is_a_tool_error(essentials_client, tmp_path):
    with pytest.raises(ToolError, match="not an existing directory"):
        await essentials_client.call_tool("validate_module", {"spec_dir": str(tmp_path / "nope")})


async def test_compile_writes_an_artifact(essentials_client, spec_dir, tmp_path):
    out = tmp_path / "out"
    result = await essentials_client.call_tool(
        "compile_module",
...
async def test_strict_compile_refuses_unresolved_rows(essentials_client, spec_dir, tmp_path):
...
async def test_compile_surfaces_warnings_on_success(essentials_client, spec_dir, tmp_path):
...
async def test_recompiling_an_untouched_spec_reproduces_the_digest(
    essentials_client, spec_dir, tmp_path
):
    first = await essentials_client.call_tool(
        "compile_module",
        {"spec_dir": str(spec_dir), "output_dir": str(tmp_path / "a"), "strict": False},
    )
    second = await essentials_client.call_tool(
        "compile_module",
...
async def test_verify_reports_an_unchecked_signature_as_unchecked(
    extended_client, spec_dir, tmp_path
):
    out = tmp_path / "out"
    ...
    result = await extended_client.call_tool("verify_artifact", {"module_dir": str(out)})
...
async def test_reverse_round_trip_preserves_the_content_signature(
    ...
    reversed_ = await extended_client.call_tool(
        "reverse_module", {"parquet_dir": str(out), "output_dir": str(rev)}
    )
    assert reversed_.data.success

    original = await extended_client.call_tool("module_signature", {"spec_dir": str(spec_dir)})
    recovered = await extended_client.call_tool(
