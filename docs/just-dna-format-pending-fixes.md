...
**Severity:** medium · **Status:** filed upstream 2026-08-19 as format-tree **`S45`**, open
(a corroboration aimed at the open half of `S41`) · **Mitigated here:** documentation only —
`draft_from_clinvar`'s docstring. We cannot detect the condition either, for the same reason
upstream's consumers cannot.
...
**Closes when** the drafter reports pre-existing rows whose identity the current predicate would no
longer produce, or upstream documents that a re-draft is additive and does not retract them.

**Not guarded by a test.** The probe monkeypatches an upstream private predicate to manufacture the
old behaviour, which is a fixture for someone else's regression rather than ours; pinning it here
would fail the day upstream renames the function and would be testing their fix, not our wrapper.
The script is `probe_s41.py`, described in the upstream note in full.
