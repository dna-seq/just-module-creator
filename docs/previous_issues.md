...
**Status (2026-09-25): resolved in 0.41.0.** Two defects, not one. Europe PMC's `/bin/` links
genuinely don't serve downloads (404/403, and PMC's `/bin/` serves a bot-challenge page). Separately,
`HttpService._get` joined a full URL onto the ESM service's base, so `fetch_supplementary` asked
`https://static-content.springer.com/https://europepmc.org/…` and could fetch from no host except
Springer. Now: `supplementary.resolve_url` addresses each JATS name on the publisher's host (Springer
family or PLOS). `_verify` HEADs every URL before listing it, fills `size_bytes`, and sets `url: null`
for a file no host serves. `net.HttpService` takes absolute URLs and has `head()`, so the Springer
probe no longer downloads the file. `_host_service` paces each host separately. The fetch note words a
403 as "no such object" only on Springer's store. A web page returned in place of a file is refused.
PLOS (`10.1371`) has a pattern rung. Live, same day: PMID 28448500 lists 27 of 27 files with PLOS URLs
and sizes, and `.s006` downloads as `pgen.1006528.s006.xlsx` (84,754 bytes).
`tests/test_supplementary.py`: six of the new tests fail on 0.40.1.
