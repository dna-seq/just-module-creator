...
Modules are scored against an expert-curated reference on three dimensions: variant recall (which rsID–genotype pairs were recovered), citation identity (whether each PMID names the intended paper, not merely exists), and effect-direction agreement. A scorer implementing them ships with the plugin. The reference is the expert’s curation rather than any system output, since scoring authored modules against a module the same system authored measures agreement with itself.

Three measurement choices are load-bearing. Recall is reported at two grains, because a run that recovers the right variant with the wrong genotypes and one that recovers half the variants exactly are distinct failures that partial credit would average away. *Decoy rate* replaces precision: a variant absent from one curation may still be correct, whereas a decoy is one an expert designated non-associated. And a metric with an empty denominator is reported as not computed rather than as disagreement, with its threshold undefined rather than passed—a check that could not run is not a check that passed.
...
Three runs of one prompt scored against an expert-curated reference module, plugin version fixed. *n/c*: empty denominator.

</div>

Table <a href="#tab:scores" data-reference-type="ref" data-reference="tab:scores">3</a> reports three runs of one prompt on one paper against that reference, at a fixed plugin version. Facts reproduced: all three runs recovered the variant the paper supports, cited both the paper and the earlier study it replicates, agreed with the reference on effect direction, and introduced no decoy. Two of the three matched the reference on every genotype row, including a homozygous row that asserts nothing because neither cohort observed a carrier. All three independently declined to propagate the seed paper’s own misdescription of the variant as stop-gained, which the study it cites classifies as intronic at the same coordinate.

Two caveats keep this from being a reproducibility claim, and both are visible only because the metrics are separated. The variance that remains is concentrated in one cell: the significance verdict on a $`p \approx 0.07`$ result, where two runs read *suggestive* and one *not significant* from the same number. That is a vocabulary ambiguity, reported upstream, not an authoring error—but a single aggregate score would have hidden it, and a reviewer needs it surfaced. More importantly, agreement here partly measures the guidance rather than the runs: the validator names the missing genotype explicitly, and a written rule states that a zero weight is a claim while a blank is not. Convergence therefore shows the workflow is prescriptive enough to produce consistent output from independent runs, which is weaker than, and should not be read as, independent confirmation that the output is correct.

Three runs of one prompt on one paper is a demonstration rather than a performance estimate. Two further papers have no reference; there the scorer reports what a module asserts against what it withholds. That reading corrected our own prediction that a paper running no association test should yield no variant rows: a run produced sixty, each recording the observation with direction and significance withheld, which is the appropriate output. Row count is therefore not scored; whether the cells assert more than the source supports is.
