...
Table <a href="#tab:scores" data-reference-type="ref" data-reference="tab:scores">3</a> reports three runs of one prompt on one paper against that reference, at a fixed plugin version. The reference holds where a curation should: no run introduced a decoy, and each recovered the variant the paper supports. Divergence is on judgement rather than retrieval. The assigned paper’s longevity claim replicates an earlier study, and retrieving that earlier paper unprompted—which the reference cites—is not stable across runs, so citation recall separates runs that a comparison of variant rows alone would call equivalent. Genotype coverage differs where the source observed no carriers, an authoring decision each run makes independently.

Three runs of one prompt on one paper is a demonstration rather than a performance estimate. Two further papers have no reference; there the scorer reports what a module asserts against what it withholds, which corrected our own prediction: we expected a paper running no association test to yield no variant rows, and a run produced sixty, each recording the observation with direction and significance withheld. The rows are appropriate and the prediction was wrong. Row count is therefore not scored; whether the cells assert more than the source supports is.
