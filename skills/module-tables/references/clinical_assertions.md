...
> each docs build, at <https://just-dna.life/just-dna-compiler/tables/clinical_assertions/>, with the authoring
> prose upstream keeps in `docs/TABLES.md` spliced above it. In-session the same answer is live from
> `describe_table("clinical_assertions.csv")` and `table_requirements("clinical_assertions.csv")`. **This file keeps the half a model
...
(`enricher/src/just_dna_enricher/clinical.py`) and `clinvar_draft.draft_gene_panel` used it as a
*drafting filter* (default 2) and kept nothing (`clinvar_draft.py`, `:516`). The consequence:
"a compiled module flattened a one-star single submission and a practice guideline to the same
`clin_sig`" (`schema/src/just_dna_format/assertions.py`). Its audience is a consumer that wants
...
more (`compiler.py _cross_check_clinical_assertions`).

## Identity card

| | |
|---|---|
| Model + module | `just_dna_format.assertions.ClinicalAssertionRow` (`schema/src/just_dna_format/assertions.py`) |
| Parquet | `clinical_assertions.parquet` — in `ARTIFACT_PARQUETS` (`compiler.py`), so inside `artifact.digest`. Registered in `_FACT_TABLES` (`compiler.py`) |
| Natural / dedup key | `(variant_key, variation_id)` — the enricher's merge key (`enricher/assertions.py`, `:302`); a `not_found` row is carried under `(variant_key, None)` (`:283`). **Not enforced by the compiler** — `_TABLE_DUPE_KEYS` (`compiler.py`) covers authored table kinds only |
| Authored or machine-produced | **machine-produced, human-overridable by design.** Standalone `BaseModel`, not an `AuthoredModel`; `extra="forbid"` so a typo'd column is refused rather than dropped (`assertions.py`) |
| Who writes it | `just_dna_enricher.assertions.enrich_clinical_assertions` — `just-dna-enricher assertions <dir>` (`enricher/cli.py`). **One producer, and no MCP tool** — see Gotcha 2 |
| Fact signature | `integrity.clinical_assertion_signature` (`schema/src/just_dna_format/integrity.py`) over `assertions.CLINICAL_ASSERTION_FACT_FIELDS` (`assertions.py`) — **13 of 17 fields**. Out: `rsid`, `source`, `status`, `fetched_at` → `manifest.clinical_assertions.signature` |
...
| Manifest block | `manifest.clinical_assertions` = `ClinicalAssertions` (`schema/manifest.py`), built by `compiler.py _clinical_assertions_block`; **absent** (`None`) when the module carries no such sidecar |
| Licence layer | `clinical_assertion` ∈ `vocab.VALID_SOURCE_LAYERS` (`schema/vocab.py`) — fact-class, so it never taints a module the way `annotation` does |
...
- **enricher pass — every column.** `enrich_clinical_assertions` (`enricher/assertions.py`,
  `just-dna-enricher assertions <dir>`) fills all seventeen. Its input is **`resolution.csv`, not
  `variants.csv`** (`assertions.py`, refuses with "run `just-dna-enricher enrich` first"), because
  ...
  agreeing with itself" (`enricher/assertions.py`).
  ...
  - `rsid` — **from the module's own `resolution.csv`, not from ClinVar.** The lookup is allele-exact
    and returns no rsID at all (`assertions.py`). This is why it sits outside the fact set.
  - `clin_sig` / `clin_sig_raw` / `review_status` / `review_stars` / `condition` / `variation_id` —
    straight from `clinvar.lookup_clin_sig` (`enricher/clinvar.py`), which returns a **list** per
    allele ordered best-reviewed first (`ORDER BY … review_stars DESC, variation_id`, `clinvar.py`).
  - `genome_build` — pinned to `ASSERTION_GENOME_BUILD = "GRCh38"` (`assertions.py`), never copied
    from the resolution row.
  - `dataset` — the snapshot's own `release.json` (`snapshot_dataset`, `assertions.py`);
...
- **author** — no column is *expected* of a human and every column *may* be written by one.
  `source` is open and names `clinvar|manual|reversed` (`assertions.py`), so `manual` is the
...
- **drafter** — **none for this table.** `clinvar_draft` drafts `variants.csv` / `studies.csv` and
  writes the star rating into free prose there instead (`clinvar_draft.py`, e.g.
...
- **registry-stamped** — nothing. The registry parses the CSV header only, to drop unknown columns on
  an upgrade (`just-dna-registry/src/just_dna_registry/services/upgrade.py`, `:471`, `:478`); it
  interprets no cell.
...
1. **Is this table inside `content_signature`?** No. `content_signature` reads only
   ...
   (`compiler.py`). Its identity is instead the **fact** hash
   ...
   ClinVar-filled table carrying the same assertions hash equal", `assertions.py`) and **`rsid`**,
   ...
   (`integrity.py`). `review_status` **and** `review_stars` are both inside although one
   determines the other — the mapping is a ClinVar convention this tier does not hold, "so the two
   columns are independent inputs here" (`assertions.py`).
2. **Is it inside `artifact.digest`?** Yes, via `clinical_assertions.parquet` in `ARTIFACT_PARQUETS`
   (`compiler.py`) — confirmed present in `manifest.artifact.files` on the compiled example. So a
   provenance-only edit no signature sees still moves the digest, because the bytes differ (row 4
   above). The CSV is additionally byte-hashed into `manifest.derived[]` (`compiler.py`) —
...
3. **Does an edit here un-close the module?** **No.** The attestation binds
   `compiler.authored_input_entries` (`compiler.py`), which is `_INPUT_FILES` newline-normalized —
   ...
   (`compiler.py`). Measured: every mutation of this file above left `verification` published;
...
- **It requires `resolution.csv`.** The pass refuses without one — *"no resolution.csv in {dir} — the
  ...
  (`enricher/assertions.py`). Which in turn means the module must already have variants with
...
- **It drags in a licence row.** On every successful write the pass calls `record_source_terms(…,
  "clinical_assertion", …)` (`assertions.py`), adding a `clinvar` / `clinical_assertion` row to
...
- **It requires a ClinVar snapshot.** With none reachable the pass is a **no-op with a warning**, not a
  failure (`assertions.py`) — see Gotcha 6.
...
- **`review_stars`** — the whole reason the table exists, and **tri-state**. `None` means the archive
  ...
  where none was read" (`enricher/clinvar_build.py`). Stored rather than derived, inverting
...
- **`review_status`** — ClinVar's verbatim wording, **open, not a vocabulary**: "it is ClinVar's
  ...
  unloadable for no gain" (`assertions.py`). Read it, never match on it exactly.
...
- **`genome_build`** — **load-bearing, not decorative.** The snapshot's lookup key is
  ...
  returning a different variant's clinical call under this module's key (`assertions.py`).
- **`alt`** — **one** allele, singular, unlike `resolution.csv`'s comma-joined `alts`, "because a
  clinical call is per-allele" (`assertions.py`).
...
- **`status`** — `not_found` is a **fact**: "the archive was consulted and has no record for this
  ...
  (`assertions.py`). Do not read an absent row as a negative.
...
weights key)"* (`assertions.py`). Measured on the compiled `hboc_palb2`: **4 of 24** assertion
keys match any weights key, and **4 of 18** weights rows have a matching assertion key. The reason is
`base.derive_variant_key` case 1 — *"an rsid row keeps its rsid, unchanged"* (`base.py`) — and
...
own cross-check does (`compiler.py`). Candidate upstream doc defect — see the summary.

**2 — the authoring plugin cannot produce this table, and cannot describe it.**
`src/just_module_creator/tools/passes.py` — `_FACT_PASSES = ("frequencies", "gene_metrics",
...
resolved allele has no ClinVar record (`assertions.py`), and the error says so itself: *"Most
...
are never queried (`assertions.py`), reported in `off_build` — deliberately **kept apart from
...
(`assertions.py`). The signal is one aggregated `logger.warning` naming the build, plus a
yellow stderr line from the CLI (`enricher/cli.py`). `reference_examples/cyp2c9_warfarin_grch37`
...
never reach an allele the table already mentioned" (`assertions.py`). And a `not_found` row is
...
exists" (`assertions.py`). A `resolved` row, and any row a curator wrote, both survive.
...
`skipped_no_snapshot=True` (`assertions.py`). Same outcome for a snapshot that is present but
...
(`clinvar_draft.py`); verified on `hboc_palb2`'s `variants.csv`, every row carries one. That
...
(`schema/assertions.py`), which reads as though conditions split rows. Measured on `hboc_palb2`:
...
returns `([], warnings)` unconditionally (`compiler.py`), so the only output is the
...
- **No `submitter` column.** `GeneValidityRow` has one and keys on it, because GenCC is an aggregate
  of nineteen submitters (`schema/gene_validity.py`). ClinVar's submitter set is not projected
...
- **No `ClinicalAssertionsInfo` on the registry's module detail.** `GwasEffects` gets one (row counts,
  units, traits — `just-dna-registry/src/just_dna_registry/services/catalog.py`); this table
...
- `just-dna-pipelines/src/just_dna_pipelines/annotation/hf_modules.py` —
  ...
  build a URL. `annotation/hf_assets.py` iterates that list, so the parquet is never downloaded.
- `annotation/report_logic.py` scans `weights.parquet`; `:575-580` joins only annotations and
...
- `v1_port/publish.py` — `_ALLOW_PATTERNS = [*ARTIFACT_PARQUETS, …]`, so the parquet **is uploaded
  to HF** (`:136`) and presence-checked (`:87`), never opened. `tests/test_format_0_6.py`
  asserts membership only.
- `v1_port/clinvar_panel.py`, `:461`, `:507` etc. do read `review_stars` — but against the
...
**just-dna-registry**
- `db/facets.py` — `"has_clinical_assertions": int(manifest.clinical_assertions is not None)`.
  Column `db/schema.py`; filter `db/repository.py`/`:1019`; API query param
  `api/routers/modules.py`; client param `client.py`/`:341`.
- `services/catalog.py` — `clinical_assertions=manifest.clinical_assertions is not None` into
  `FactTablesInfo` (`models/api.py`). **A yes/no tick on the card, nothing else.**
- `specfiles.py` — `clinical_assertions.csv` ∈ `FACT_CSVS`, so revalidate/upgrade rebuild the spec
  dir with it. `services/upgrade.py`/`:471`/`:478` reads the CSV **header** to trim unknown
  columns; no cell is interpreted. `services/publish.py` and `revalidate.py` call
...
- **No `min_review_stars` / `max_review_stars` / `unrated_count` / `not_found_count` / `variation_id`
  ...
  `latest_manifest` passthrough (`models/api.py`).

**just-prs / just-prs-mcp** — zero hits, both repos.

**just-module-creator** — no read. `tools/authoring.py` names the file in a string telling the
model not to hand-author it; `verify_artifact` re-hashes the parquet by digest without opening it;
`tools/passes.py` threads `min_review_stars=2` into `draft_from_clinvar`, which filters the
...
before.** `_clinical_assertions_block` (`compiler.py`) builds the signature, the counts and
...
- **Add `clinical_assertions` to `MODULE_TABLES` and `ModuleTable`**
  (`annotation/hf_modules.py`, `:495-503`). Unread today; the parquet is already published to HF by
  `v1_port/publish.py`, so this is a one-line list change plus a `ModuleTable` member, not a
  pipeline change. What breaks today: the annotator cannot see the table even when the module ships
  one.
- **Render review strength beside "ClinVar interpretation."** `report_logic.py` prints
...
- **Resolve `clin_sig` per observed ALT, not per variant row.** `_effective_clin_sig`
  (`report_logic.py`) reads one flat authored value. On `hboc_palb2` that value is
  ...
  `compiler.py` does, **not** on `variant_key`, which matched only 4 of 24 in measurement —
...
- **Surface `conflicting` as its own state rather than a `clin_sig` token.** `_clin_sig_label`
  (`report_logic.py`) renders it as one more tier. `hboc_palb2` carries one
...
- **Registry: build a `ClinicalAssertionsInfo` on `ModuleDetail`.** `GwasEffectsInfo` already exists
  (`services/catalog.py`) and `models/api.py` states the intent — "the counts and
...
- **Do not average the stars.** The manifest publishes min and max as two counts deliberately —
  ...
  (`schema/manifest.py`) — and `min`/`max` are `int | None` where `null` is not `0`
  (`compiler.py`). Any UI that coalesces null to zero reports a module's evidence as the
