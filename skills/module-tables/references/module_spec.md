...
| | |
|---|---|
| Model + module | `just_dna_format.spec.ModuleSpecConfig`, with `ModuleInfo(Display)` and `Defaults` in the same file. `Weighting`, `Contribution`, `GenePanelSpec` and `Display` live in `just_dna_format.manifest` and are imported (`spec.py`). |
| Parquet it becomes | **None.** It becomes `manifest.json` (`manifest.ModuleManifest`, built at `compiler.py`). Two of its values reach `weights.parquet` anyway — `module.name` as the `module` column, and `defaults:` folded into `curator`/`method`/`priority` per row. |
...
| Authored or machine-produced | **Authored, entirely.** No drafter and no enricher pass writes it. `just-dna-compiler scaffold` writes a skeleton (`scaffold.py`). |
...
| In `content_signature`? | **Two parts only** — `genome_build` when it is not `GRCh38` (`integrity.py`), and `defaults:` folded into each row by `_resolve_spec_defaults` (`compiler.py`, RM37). Everything else is excluded by design: *"the identity and display half of `module_spec.yaml` … is excluded"* (`integrity.py`). |
...
| In the attestation binding? | **Yes, all of it.** `module_spec.yaml` is the first member of `_INPUT_FILES` (`compiler.py`), which is what `authored_input_entries` hashes (`compiler.py`). Any byte change drops the attestation and the closure. |
...
- **drafter** — **none.** No provider writes this file. `clinvar_draft` (`clinvar_draft.py`) and `pgx_draft` (`pgx_draft.py`)
...
- **enricher pass** — **none writes it.** `enrich` reads `genome_build` out of it and will not guess:
  *"the enricher cannot know the module's declared build, so it will not choose one for you — fix
  module_spec.yaml"* (`enricher/src/just_dna_enricher/enrich.py`). A spec with **no**
  `module_spec.yaml` at all gets the format default instead (`enrich.py`).
- **compiler-stamped** — nothing in this file. The compiler *copies* it into the manifest and
  *coerces* one field: `module.version` is rewritten to SemVer in place by `ModuleInfo._enforce_semver`
  (`spec.py`), with the original kept on the non-field property `version_coerced_from`. That
...
- **registry-stamped** — `namespace`, `owner`, `canonical_id` (`normalize.IDENTITY_AUTHORITY_KEYS`).
  ...
  inputs are not permitted* (`normalize.py`). Measured — `module: {namespace: acme}` raises
  *"registry-stamped identity key(s), not authored fields"*. `version` is **deliberately absent** from
  ...
  `module:` block **on disk** (`just-dna-registry/src/just_dna_registry/services/publish.py`,
  ...
  to the default genome build, and quietly produces the wrong signature"* (`publish.py`). It
  then stamps all three back plus `published_at` and a signature (`publish.py`). The strip
  happens at publish, so **a download is already clean and is republishable as itself** —
  `tests/test_v04.py` asserts `set(module) & IDENTITY_AUTHORITY_KEYS == set()`.
- **nobody, ever** — `panel:`. Deprecated in 0.6, removed at 1.0, and **nothing reads it any more**:
  its last machine reader moved to the licence row's `dataset` column (`spec.py`, RM4). It is
...
- **`weighting` is not derivable and no tool offers to derive it.** It is three free-text strings and
  ...
  nobody has surveyed"* (`manifest.py`). Nothing can read a module's weights and report what
...
- **`authorship` may only be appended to.** A later pass adds an entry; it never edits an existing one.
  ...
  spelled out and there is no lossy `hybrid` tag"* (`manifest.py`).
...
1. **Is this table inside `content_signature`?** Only in the two places above. `genome_build` feeds the
   ...
   for byte and only the misidentified ones move (`integrity.py`). `defaults:` is folded into
   ...
   (`compiler.py`). The whole identity/display half is excluded on purpose.
...
1. **`weighting:` is invisible unless you already know it exists — and two real authoring sessions
   ...
   (`scaffold.py`). Rendered, `module_spec_template(name="demo_module")` emits **thirteen lines**
   ...
   `known_kind` appends `.csv` and refuses with *"Unknown table kind"* (`_shared.py`), and the
   ...
   **Absent means the module has not said, which is not the same as saying its weights are
   comparable** (`manifest.py`). A consumer must read absent as *do not aggregate across
...
2. **`genome_build` decides the identity key, and it is the one key here that moves content identity.**
   ...
   GRCh38 one for a locus 228 bp away (`compiler.py`). Two consequences you will meet:
   ...
   (`compiler.py`). And a typo'd `genome_bild:` is a hard error precisely so it cannot
   silently leave the default in force (`spec.py`).
3. **A digitless `version:` becomes `0.0.0` — a legal SemVer nobody wrote.** Measured on 0.6.1:
   ...
   (`spec.py`). Quote anything with a dot.
4. **Editing this file un-closes the module while moving no identity — including a display reword.**
   ...
   *"never stamped by a passing check"* (`compiler.py`).
...
7. **Because `weighting` is outside `content_signature`, two modules that differ only in it collide.**
   ...
   `(namespace, name)` with `409 duplicate_content` (`publish.py`). A weighting declaration
   ...
   (`just-dna-registry/tests/test_format_06.py`) and then pinned it deliberately at `:237`.
...
- **No precedence field inside `weighting:`.** A typed rule saying "use the GWAS effect where `weight`
  ...
  which is the defect the report is about"* (`manifest.py`). A consumer chooses a table
...
- **No versioning contract at all.** The registry enforces exactly two things — that a version parses
  as SemVer (`api/routers/publish.py`) and that `(namespace, name, version)` is new. **Ordering
  is client-side only**: `client_cli.py` refuses a version ≤ current latest, and any direct
  HTTP caller bypasses it; `recompute_latest` then just keeps the higher one as latest
  (`repository.py`). Confirms MODULE_LIFECYCLE § 6.0 — do not invent a milestone ladder.
- **No conflict check between authored `module.version` and the published version.** There is no code
  ...
  and that always wins"* (`publish.py`). Stripping it was the policy through 0.10 and was
...
- **No `verification` facet either**, for the same class of reason: *"a registry that let you sort by
  someone else's unverifiable pass would be lending it our credibility"* (`models/api.py`).
...
- `just-dna-pipelines/src/just_dna_pipelines/annotation/hf_modules.py` — `_weighting_summary`
...
- `.../hf_modules.py` — `read_module_provenance` returns `(identity.version, artifact.digest,
  weighting)`. The digest is *displayed*, never verified: nothing in that repo calls `verify_manifest`.
- `.../annotation/hf_logic.py,644` → `.../annotation/report_logic.py` →
...
- `.../hf_modules.py,153` — the **remote** (HF/HTTP) path validates the whole `ModuleManifest` and
  takes only `artifact.files[].name`.
- `webui/src/webui/state.py` — `identity.version/.namespace/.name` and
  `display.title/.icon/.color` for the local-module scan.
- `just-dna-pipelines/src/just_dna_pipelines/module_registry.py` — reads `manifest.json` as a
  raw dict and copies only the `display` block into `modules.yaml`.

**From `module_spec.yaml` directly:**

- `.../module_config.py` — `spec_version()` reads `module.version` only, coercing legacy
  `2`/`v2`, because the compiler leaves `identity.version` null until publish.
- `.../module_registry.py,46-54,137` — parses the full `ModuleSpecConfig` and uses `module.name`.
- `.../agents/module_creator.py` and `webui/state.py` — the `module:` block only
  (name/version/title/description/report_title/icon/color).
- Upstream, `just_dna_enricher/enrich.py` reads `genome_build` and refuses to guess when the
  file will not parse.
...
- `src/just_dna_registry/specfiles.py` — `REQUIRED_SPEC_FILES = (SPEC_YAML,)`. `module_spec.yaml` is
...
- **An uploaded `manifest.json` is never read.** The publish contract is spec-only and the server
  recompiles: the client refuses to send one (`client.py`, `_SKIP_UPLOAD_NAMES`) and `_finalize`
  overwrites any that arrives (`publish.py`). So *everything* the registry knows about a module
  outside its CSV rows comes from this file, through the compile it runs itself.
- `publish.py` — `validate_spec(spec_dir, IDENTITY_AUTHORITY_KEYS)`, deliberately **modeless**,
  before enrichment; then `compile_module(..., strict=settings.compile_strict)` at `:522-547`.
- **`weighting` is stored, displayed verbatim and indexed.** `services/catalog.py` (`_weighting`
  — *"verbatim, or `None` if it has not said"*) reaches `ModuleDetail.weighting`; `db/facets.py`
  projects `weighting_declared = int(manifest.weighting is not None)` into a column
  (`db/schema.py`) that `?weighting_declared=` filters on. The card carries only the boolean. The
  facet's own note: *"this says the module **stated** what its weights mean, not that it has any …
  an absent declaration answers it with *no*"* (`facets.py`), and an absent block renders as
  `null`, never as an empty one (`tests/test_format_06.py`).
- `genome_build` → a column (`db/schema.py`), a card field (`catalog.py`), and a `?genome_build=`
...
- **Gate the "Net weight" headline on `weighting`, or annotate it.** `report_logic.py` and
...
- **Carry `weighting` down the remote path.** `hf_modules.py` has the whole validated
...
- **Read `genome_build` before joining positions.** No consumer dereferences it, in the manifest or the
  spec; `cli_annotate.py` falls back to the literal `"GRCh38"` from the *sample* and
  `agents/module_creator.py` hardcodes the same string into every scaffolded spec. A GRCh37 module
...
- **Surface `authorship` as the trust signal it was built to be.** `Contribution.kind` exists so *"a
  consumer routes its scrutiny by `kind`"* (`manifest.py`). Nothing reads it, so a report
...
- **Read the module-wide `license`, or say why not.** The report reconstructs licensing from
  `sources.parquet` (`report_logic.py`) and never reads `manifest.license`, so the author's
