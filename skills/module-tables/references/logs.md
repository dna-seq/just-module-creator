...
| | |
|---|---|
| Model + module | `just_dna_format.manifest.FileEntry`, in `ModuleManifest.logs: list[FileEntry]` (`schema/src/just_dna_format/manifest.py`) |
| Parquet | **None, ever.** Logs are copied verbatim into the artifact dir; nothing materializes them |
| Natural / dedup key | `(name, sha256)` — the dedup key `aggregate_logs` uses (`schema/src/just_dna_format/aggregate.py`) |
...
| Who collects it | `just_dna_compiler.compiler._collect_logs` (`compiler/src/just_dna_compiler/compiler.py`), called at `compiler.py` |
| Fact signature | **None.** No signature of any kind covers a log's bytes |
| In `content_signature`? | **No.** `content_signature` hashes parsed CSV rows plus a non-default `genome_build` and nothing else (`integrity.py`) |
| In `artifact.digest`? | **No**, explicitly — `manifest.py`: "Kept out of `artifact.digest` so identical compiled data stays dedup-equal regardless of logs" |
| In `manifest.inputs`? | **No.** `_INPUT_FILES` is `module_spec.yaml` + `variants.csv` + `studies.csv` + the table kinds (`compiler.py`) |
| In the Ed25519 signature? | **No.** `sign_digest` signs the `artifact.digest` string's UTF-8 bytes and nothing else (`schema/src/just_dna_format/signing.py`) |
...
- **compiler-stamped** — the compiler collects and hashes, it does not author. `_collect_logs`
  ...
  `file_entries(output_dir, names)` (`compiler.py`).
- **registry-stamped** — nothing on the entry itself. But on a registry publish the **server** runs
  ...
  uploaded, not yours (`just-dna-registry/src/just_dna_registry/services/publish.py`).
...
1. **Inside `content_signature`?** No. `content_signature` takes a mapping of CSV filename → parsed
   rows and hashes their normalized JSON, plus `genome_build` when non-default (`integrity.py`).
...
2. **Inside `artifact.digest`?** No, and this is the one exception-shaped fact worth memorizing:
   ...
   regardless of what logs came with it (`manifest.py`). **Measured:** appending to `v2.log` left
   `artifact.digest` byte-identical.
3. **Does an edit here un-close the module?** No. The attestation binds `authored_input_entries`
   (`compiler.py`), which is `_INPUT_FILES` newline-normalized — `module_spec.yaml`,
...
`schema/tests/test_logs.py` pins exactly that (`verify_manifest(..., check_logs=True)` on a
...
- **`logs/` is the one subtree the registry never flattens.** `plan_layout` hoists a recognized spec
  ...
  (`just-dna-registry/src/just_dna_registry/specfiles.py`), because the manifest records that
  path verbatim and flattening would rename a file the manifest attests
  (`specfiles.py`). Arbitrary depth survives — **measured**: `logs/team/reviewer.log`
...
`manifest.json` and `WHERE-THIS-CAME-FROM.md`; `just-dna-registry/src/just_dna_registry/client.py`).
...
on disk` (`integrity.py`). Consequence: **anyone can strip every log from a downloaded module
and it still verifies.**

**3. Nothing in this plugin turns `check_logs` on.** `verify_artifact` calls `verify_manifest` with
only `require_marketplace` and `public_key` (`src/just_module_creator/tools/authoring.py`).
So through our surface a tampered log is undetectable. `just-dna-compiler verify --check-logs` is the
only shipped route (`compiler/src/just_dna_compiler/cli.py`).
...
(`just-dna-lite/just-dna-pipelines/src/just_dna_pipelines/v1_port/runner.py`). The repair was
...
explicit branch or the discovery branch, never both (`compiler.py`). The v1_port runners each
pass exactly one file (`runner.py`, `pharmgkb_runner.py`, `clinvar_runner.py`), so a
directory holding both `v1_port.log` and `clinvar_panel.log` ships only the one named. That pipeline
works around it by *deleting* the other on rebuild (`clinvar_panel.py`, so a rebuild does not
...
from `RECOGNIZED_SPEC_FILES ∩ storage` (`services/upgrade.py`), and no log spelling is in
`RECOGNIZED_SPEC_FILES` (`specfiles.py`). The comment states the intent: *"Logs and provenance are
intentionally NOT carried: they describe how the predecessor was built, and this mechanical
re-publish has its own (absent) provenance"* (`upgrade.py`). So an upgraded successor has
...
server-side: `tests/test_v05.py` publishes a second version with only `v2.log` and asserts
...
(`aggregate.py`). "First occurrence wins" in its docstring is about *which FileEntry object* is
...
0.18.2). That filter is on the *packed dry-run* path only (`services/publish.py`); the real
publish extracts unfiltered, and both shapes reach `manifest.logs` — `tests/test_v05.py`
...
no zone (`agents/module_creator.py`). Every real transcript starts that way
...
- **No `--log-file` flag on `just-dna-compiler compile`.** The parameter exists in the Python API
  (`compile_module(log_files=...)`, `compiler.py`); the CLI exposes only discovery. Checked
  against the whole option list at `compiler/src/just_dna_compiler/cli.py`.
- **No log surface in this plugin at all.** `scaffold_module` creates no `logs/` directory,
  ...
  (`src/just_module_creator/models.py`), and `verify_artifact` never sets `check_logs`. An
...
- **No schema, no version field, no declared format.** Grepping every repo for `v0.1` finds only
  ...
  - the Module Manager's transcript — header, `====` rule, then `[%7.1fs] message` lines with
    ...
    (`module_creator.py`, `_fmt_detail:152-160`);
  - v1_port's `key: value` provenance card — `module:`, `source_sha256:`, `variant_rows:`,
    `min_review_stars:`, `warnings:` (`v1_port/writer.py`, `v1_port/clinvar_panel.py`).
...
- **No `check_logs` by default, and that is the decision.** `verify_manifest`'s step 5 spells out
  ...
  (`integrity.py`). This is the `logs` rule; a repair making it strict would break every
...
- **`logs` was the precedent, not a candidate for change.** When we asked for `manifest.derived`
  ...
  (`just-dna-format/docs/CONSUMER_SUGGESTIONS_HISTORY.md:242-244`, shipped at `manifest.py`).
...
|---|---|---|---|---|---|---|
| `authorship:` | one contributor, one version | inside `module_spec.yaml` → `manifest.authorship` | no | **no** (`manifest.py`) | **yes** — it is inside `_INPUT_FILES` | no |
| `verification.json` | one *check*, with counts and the release checked against | beside the spec → `manifest.verification` + `manifest.derived` | no (`manifest.py`) | no | no — but a stale binding **drops the block** | no |
...
- **Who contributed, and in what capacity → `authorship:`.** `Contribution` is three orthogonal axes
  (`manifest.py`): `who` (free text — a name, handle or model id), `role` (a **closed**
  ...
  plus a SOTA swarm?* (`vocab.py`). Call `authoring_reference` for the live members; do not
...
- **What was checked, and did it pass → `verification.json`.** Written by the enricher, bound to the
  ...
  an edited attestation is not stale but false (`manifest.py`). Never hand-write one.
- **Why this row says what it says → `provenance.json`.** `ProvenanceItem` is per-variant:
  ...
  (`manifest.py`). This is where an AI co-author's *reasoning about a specific call* belongs,
...
(`agents/module_creator.py`, `create_module_team:618`) — with four PI tools:
...
**What it writes as a log.** `RunLog` (`module_creator.py`) accumulates status messages,
...
team system prompt verbatim. `_write_run_log` (`webui/src/webui/state.py`) writes
...
1. agent run → `GENERATED_MODULES_DIR/<name>/v<N>/v<N>.log` (`state.py` persists, then
...
2. `register_custom_module` copies `.log` into `CUSTOM_MODULES_DIR/<name>` — top level only, no
   recursion (`just-dna-pipelines/src/just_dna_pipelines/module_registry.py`).
3. `RegistryClient.publish(..., CUSTOM_MODULES_DIR / key)` (`state.py`) →
   `gather_spec_files` uploads it (`client.py`).
4. Server compiles; `_collect_logs` discovers it; `manifest.logs` gets the entry
   (`publish.py`; proven by `tests/test_v05.py`).
...
(`v1_port/runner.py`, `pharmgkb_runner.py`, `clinvar_runner.py`). Its comments are the only
two places in any consumer repo that mention `manifest.logs` (`clinvar_panel.py`,
`runner.py`), and it treats the log as the module's provenance record — recording the *actual*
...
|---|---|---|
| registry server | `just-dna-registry/src/just_dna_registry/api/routers/modules.py` | `GET .../versions/{v}/logs` lists name, sha256, size and a fetch URL |
| registry server | `modules.py` | `GET .../files/{path}` serves any manifest-listed file; a log fetch is explicitly **not** counted as a download |
| registry server | `modules.py` | the version tarball includes `manifest.logs` entries |
| registry server | `services/publish.py` | records `logs=[e.name for e in manifest.logs]` in the structured publish action |
| registry client | `client.py` | `RegistryClient.logs(namespace, name, version)` |
| registry CLI | `client_cli.py` | prints `logs: <names>` after a download |
| just-dna-lite | `webui/src/webui/app.py` | `/api/agent-log/{name}/{vdir}/{file}` serves a **local generated** log by filesystem path — never from a manifest |
| just-dna-lite | `webui/src/webui/state.py` | `slot_archive_logs` lists local `v*.log` across version dirs for the editing slot |
| just-dna-pipelines | `v1_port/clinvar_panel.py`, `v1_port/runner.py` | comments only — they *write* logs knowing they will be hashed |
...
- **The Module Manager writes `v<N>.log` and never `logs/<role>.log`, though it runs a named team.**
  ...
  consumer can fetch just the review. **Blocked at two hops**: `state.py` and
  `module_registry.py` both iterate `iterdir()` with `is_file()`, so a `logs/` subtree is
...
- **Nobody calls `aggregate_logs`, so cross-version provenance is asserted and never assembled.**
  ...
  `registry upgrade` deliberately does not carry logs forward (`upgrade.py`), so the union
...
- **`RunLog` stamps naive local time.** Ask for ISO-8601 UTC in the header
  (`module_creator.py`), so two logs from two machines are comparable and sort against every
