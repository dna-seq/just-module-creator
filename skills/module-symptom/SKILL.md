...
**The lookup is `../module-101/references/SYMPTOMS.md`.** Real message text → cause → action, in four
sections: authoring and loading, resolution and enrichment, validation and compile, and the checks. It
...
| **us** | a path outside the workspace, an offline refusal, a tool that is not in this tier, an unauthenticated result | a setting, never a row. Nothing in your CSVs will fix it |
