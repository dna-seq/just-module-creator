...
an error**, so a module ships without one unnoticed. `sources.csv` is the deprecated spelling; both
spellings still read, only `licensing.csv` is created, and a module carrying **both** is
